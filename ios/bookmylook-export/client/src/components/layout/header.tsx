...
import { Menu, Home, ArrowLeft, LogOut, User, MapPin, Calendar, Info, Mail, HelpCircle, FileText, Share2, History, Shield, ShoppingBag, LayoutGrid, Settings, Edit, Star, Trash2, Sun, Moon, Monitor } from "lucide-react";
    ...
    <header className="bg-gradient-to-r from-white/95 via-rose-50/80 to-purple-50/80 dark:from-gray-900/95 dark:via-gray-800/80 dark:to-gray-900/80 backdrop-blur-md shadow-lg shadow-rose-500/10 dark:shadow-gray-900/30 sticky top-0 z-50 border-b border-rose-100/30 dark:border-gray-700/30">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-auto py-4">
          ...
          {/* Mobile navigation - Provider gets hamburger, clients use MobileNavigationNew */}
          {isProvider ? (
            // Provider hamburger menu (both mobile and desktop)
              ...
              <Sheet open={isOpen} onOpenChange={setIsOpen}>
                ...
                <SheetContent side="right" className="w-72 overflow-y-auto">
                  ...
                  <div className="mb-2">
                    ...
                    {/* Only show Dashboard in hamburger menu on tablets/iPads (md and above) */}
                    {/* On phones, they appear in bottom navigation instead */}
                    <div className="hidden md:block">
                      {/* Show different navigation based on user role */}
                      {(() => {
                        const userRole = client?.role || localStorage.getItem('userRole');
                        const isProvider = userRole === 'provider' || userRole === 'professional';
                        
                        return isProvider ? (
                          <Link href="/provider-dashboard" onClick={() => setIsOpen(false)}>
                            <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-100 transition-colors" data-testid="nav-dashboard">
                              <LayoutGrid className="h-5 w-5 text-gray-700" />
                              <span className="text-gray-900 font-medium">Provider Dashboard</span>
                            </div>
                          </Link>
                        ) : (
                          <Link href="/my-bookings" onClick={() => setIsOpen(false)}>
                            <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-100 transition-colors" data-testid="nav-my-bookings">
                              <Calendar className="h-5 w-5 text-gray-700" />
                  ...
                  {/* Provider Dashboard - Show only for authenticated providers */}
                  {isProvider && (
                    <>
                      <div className="mb-2">
                        <p className="px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Provider Actions</p>
                        
                        <button
                          onClick={() => {
                            setIsOpen(false);
                            // Trigger edit profile modal - will need to pass this via context or state
                        ...
                        <button
                          ...
                          data-testid="nav-provider-logout"
                        >
