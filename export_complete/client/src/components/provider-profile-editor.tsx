...
function LinkedAccountSection({ providerId, provider }: { providerId: string; provider: any }) {
  ...
  const hasUpi = provider.upiId && provider.upiId.trim() !== '';
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          {hasUpi && (
            <Badge className="bg-green-500 text-white text-xs">UPI Configured</Badge>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {hasUpi ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-600">
              ...
              <span className="font-medium">UPI Payment Ready</span>
            </div>
            <p className="text-sm text-gray-600">
              Your UPI ID is configured: <span className="font-mono text-professional-teal">{provider.upiId}</span>
            </p>
            <p className="text-sm text-gray-600">
              When clients pay online, you receive your full service amount automatically via UPI within 24 hours. 
              The 3% platform fee is paid by the client on top of your service price.
            </p>
            <div className="bg-green-50 p-3 rounded-lg text-sm">
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                <span className="text-green-800 font-medium">Payment setup complete!</span>
              </div>
            </div>
          </div>
        ) : (
            ...
            {!hasUpi ? (
              <div className="flex items-start gap-2 text-yellow-600 bg-yellow-50 p-3 rounded-lg">
                ...
                <div className="text-sm">
                  <p className="font-medium">UPI ID required</p>
                  <p className="text-gray-600 mt-1">
                    Please add your UPI ID (PhonePe/GPay/Paytm) in the Payment Information section above to receive automatic payments.
                  </p>
                </div>
              </div>
            ) : (
...
export default function ProviderProfileEditor({ provider }: ProfileEditorProps) {
  ...
  // Reset all forms when provider data changes (fixes mobile/TestFlight bug)
  useEffect(() => {
    businessForm.reset({ businessName: provider.businessName || "" });
    descriptionForm.reset({ description: provider.description || "" });
    locationForm.reset({ location: provider.location || "" });
    staffCountForm.reset({ staffCount: provider.staffCount || 1 });
    paymentForm.reset({
      bankName: provider.bankName || "",
      accountHolderName: provider.accountHolderName || "",
      accountNumber: provider.accountNumber || "",
  ...
  }, [provider, businessForm, descriptionForm, locationForm, staffCountForm, paymentForm, scheduleForm, staffNamesForm, servicesForm]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Payment Information - UPI Only (Simplified) */}
      {renderEditableField(
        "payment",
        "Payment Information",
        provider.upiId || "Not configured",
        paymentForm,
        ...
        () => (
          <div className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>UPI ID (For Automatic Payouts)</FormLabel>
                  <FormControl>
                    <Input 
                      {...field} 
                  ...
                  <p className="text-xs text-gray-500">
                    💡 When clients pay online, your earnings are automatically transferred to this UPI ID within 24 hours
                  </p>
                </FormItem>
              )}
            />
