...
import { sendCashfreePayout, sendCashfreeUpiPayout, getCashfreePayoutStatus, isCashfreeConfigured } from "./cashfree";
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      await db.transaction(async (tx) => {
        ...
        if (allBookingIds.length > 0) {
          for (const bookingId of allBookingIds) {
            // Delete provider payouts for this booking (MUST BE FIRST - references bookings)
            await tx.delete(providerPayouts).where(eq(providerPayouts.bookingId, bookingId));
          ...
          if (booking) {
            ...
            if (servicePrice > 0) {
              ...
              if (hasUpi || hasBankDetails) {
                ...
                console.log(`[INSTANT PAYOUT] Provider has ${paymentMethodType}, initiating transfer`);
                
                // Try Cashfree first (preferred), fallback to RazorpayX if not configured
                const useCashfree = isCashfreeConfigured();
                console.log(`[INSTANT PAYOUT] Using ${useCashfree ? 'Cashfree' : 'RazorpayX'} for ${paymentMethodType} payout`);
                
                let payoutResult: any;
                let paymentMethod = 'manual';
                
                if (useCashfree && hasUpi) {
                  // PREFERRED: Use Cashfree UPI Payout (instant, 24x7, simpler)
                  console.log(`[INSTANT PAYOUT] Using Cashfree UPI payout to ${provider.upiId}`);
                  ...
                  // Use Cashfree Bank Account Payout (99.98% success, instant IMPS)
                  console.log(`[INSTANT PAYOUT] Using Cashfree bank transfer to ${provider.accountNumber}`);
                  ...
                  console.log(`[INSTANT PAYOUT] Using RazorpayX bank transfer to ${provider.accountNumber}`);
  ...
  app.post("/api/bookings", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // Check if appointment is at least 5 minutes in advance (allow last-minute bookings if slot is available)
      const now = new Date();
      ...
      if (appointmentDateTime < fiveMinutesFromNow) {
        console.log('❌ Booking too soon:', {
          ...
          minimumTime: fiveMinutesFromNow.toISOString(),
          currentTime: now.toISOString()
        });
        return res.status(400).json({ 
          error: "Appointment must be scheduled at least 5 minutes in advance",
          details: "Please select a time slot that is at least 5 minutes from now"
        });
      }
  ...
  app.get("/api/provider/:providerId/appointments", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (date) {
        // Parse date in IST (India Standard Time, UTC+5:30) to match how bookings are stored
        // Frontend sends YYYY-MM-DD format, we need to interpret it as IST date
        const [year, month, day] = (date as string).split('-').map(Number);
        
        // Create IST midnight for the requested date
        ...
        console.log(`📅 [APPOINTMENTS] Querying for date ${date} (IST range: ${startOfDayIST.toISOString()} to ${endOfDayIST.toISOString()})`);
        
        whereCondition = and(
          eq(bookings.providerId, providerId),
          sql`${bookings.appointmentDate} >= ${startOfDayIST}`,
          sql`${bookings.appointmentDate} <= ${endOfDayIST}`
        )!;
      }
  ...
  app.get("/api/provider/:providerId/availability/:date", async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      console.log(`🔍 [AVAILABILITY] Provider ${providerId} requesting availability for ${date}`);
      
      // Parse the date and get start/end of day
      const startOfDay = new Date(date + 'T00:00:00.000Z');
      const endOfDay = new Date(date + 'T23:59:59.999Z');
      
      console.log(`📅 [AVAILABILITY] Date range: ${startOfDay.toISOString()} to ${endOfDay.toISOString()}`);
      
      // Get the day of week (0 = Sunday, 1 = Monday, etc.)
      const targetDate = new Date(date);
      const dayOfWeek = targetDate.getDay();
      
      console.log(`📆 [AVAILABILITY] Day of week: ${dayOfWeek} (0=Sunday, 6=Saturday)`);
      ...
      console.log(`📋 [AVAILABILITY] Found ${schedules.length} total schedules for provider`);
      
      const daySchedules = schedules.filter(s => s.dayOfWeek === dayOfWeek && s.isAvailable);
      console.log(`📋 [AVAILABILITY] Found ${daySchedules.length} schedules for day ${dayOfWeek}`);
      ...
      console.log(`👥 [AVAILABILITY] Found ${staffMembersData.length} active staff members`);
      ...
      console.log(`📊 [AVAILABILITY] Found ${dayBookings.length} bookings for this day:`, dayBookings.map(b => ({ token: b.tokenNumber, date: b.appointmentDate, status: b.status })));
      ...
      const bookingsWithDetails = await Promise.all(
        dayBookings.map(async (booking) => {
          ...
          // CRITICAL: Database stores times in UTC, but schedules use IST (India Standard Time)
          // We need to convert UTC booking times to IST for proper slot matching
          ...
          // Convert UTC to IST (UTC+5:30) for slot matching
          // Get UTC time in milliseconds, add IST offset (5 hours 30 minutes = 19800000 ms)
          const istOffset = 5.5 * 60 * 60 * 1000; // 5 hours 30 minutes in milliseconds
          const startTimeIST = new Date(startTime.getTime() + istOffset);
          const endTimeIST = new Date(endTime.getTime() + istOffset);
          
          // Extract IST hours/minutes for slot matching
          const localStartHour = startTimeIST.getUTCHours(); // Use getUTCHours since we manually added offset
      ...
      const isSlotAvailable = (slotStartMinutes: number, staffId: string, serviceDurationMinutes: number = 15) => {
        ...
        for (const booking of bookingsWithDetails) {
          ...
          // FIXED: Use pre-calculated IST time components instead of UTC
      ...
      for (let minutes = startMinutesFromMidnight; minutes < endMinutesFromMidnight; minutes += 15) {
        ...
        const staffSlots = staffMembersData.map(staff => {
          ...
          // CRITICAL FIX: Only attach booking object if this slot is the START time of the booking
          // Don't attach it to every overlapping slot (prevents duplicate display)
          ...
          if (primaryAvailability.reason === 'booked' && primaryAvailability.booking) {
            ...
            // Only include booking if this slot's start time matches the booking's start time
        ...
        timeSlots.push({
          ...
          bookings: bookingsWithDetails.filter(booking => {
            // FIXED: Use IST time components instead of UTC
      ...
      // PROVIDER DASHBOARD: Show ALL bookings for the day, including past ones
      // Providers need to see their complete day schedule, not just future appointments
      const now = new Date();
      const isToday = date === now.toISOString().split('T')[0];
      
      // For slot counting (availability), only count future slots
      const futureSlots = isToday 
        ? hourlyAvailability.filter(slot => {
            ...
            // Only include slots that are at least 5 minutes in the future (allow last-minute bookings)
            return slotMinutesFromMidnight >= currentMinutesFromMidnight + 5;
          })
        : hourlyAvailability; // For future dates, count all slots
      
      // CRITICAL FIX: Return ALL bookings for provider dashboard, not just future ones
      // Providers need to see their full day schedule including completed appointments
      ...
      res.json({
        date,
        totalBookings: allDayBookings.length, // Show ALL bookings for the day
        ...
        hourlyAvailability, // Contains all bookings embedded in staffSlots
        staffMembers: staffMembersData,
        schedule: {
  ...
  // Update UPI payment details for provider (simpler than bank details)
  app.put("/api/providers/:id/upi-details", async (req, res) => {
    try {
      ...
      console.error("UPI details update error:", error);
      res.status(400).json({ message: error instanceof Error ? error.message : "Failed to update UPI details" });
    }
  });

  // Bulk update provider services from template
  app.put("/api/providers/:id/services-from-template", async (req, res) => {
    try {
      ...
      // Delete all existing services for this provider
      ...
      if (services.length > 0) {
        ...
        await db.insert(providerServiceTable).values(servicesToInsert);
      }
      ...
      console.error("Services template update error:", error);
      res.status(400).json({ message: error instanceof Error ? error.message : "Failed to update services" });
    }
  });
  ...
  app.post("/api/providers/:id/create-linked-account", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // NOTE: Function not implemented - Route not in use
  ...
  app.get("/api/providers/:id/linked-account-status", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // NOTE: Razorpay Route not in use - using RazorpayX instant payouts instead
  ...
  // Carousel Images - Public endpoint to get active images with location filtering
  app.get("/api/carousel-images", async (req, res) => {
    try {
      const { stateId, districtId, townId } = req.query;
      
      // Build filter condition for location-based images
      // Show images that either:
      // 1. Have no location set (null = show globally)
      // 2. Match the most specific location provided by the user
      
      let whereCondition;
      
      if (townId) {
        // User has town-level location - show global images OR images for this town
        whereCondition = and(
          eq(carouselImages.isActive, true),
          or(
            isNull(carouselImages.townId), // Global images (no town filter)
            eq(carouselImages.townId, townId as string) // Town-specific images
          )
        );
      } else if (districtId) {
        // User has district-level location - show global images OR images for this district
        whereCondition = and(
          eq(carouselImages.isActive, true),
          or(
            isNull(carouselImages.districtId), // Global images (no district filter)
            eq(carouselImages.districtId, districtId as string) // District-specific images
          )
        );
      } else if (stateId) {
        // User has state-level location - show global images OR images for this state
        whereCondition = and(
          eq(carouselImages.isActive, true),
          or(
            isNull(carouselImages.stateId), // Global images (no state filter)
            eq(carouselImages.stateId, stateId as string) // State-specific images
          )
        );
      } else {
        // No location provided - show only global images (no location filter)
        whereCondition = and(
          eq(carouselImages.isActive, true),
          isNull(carouselImages.stateId),
          isNull(carouselImages.districtId),
          isNull(carouselImages.townId)
        );
      }
